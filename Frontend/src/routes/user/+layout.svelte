<script lang="ts">
	...
	import { userNavItems, getPageTitle } from '$lib/config/navigation';
	import NavigationSidebar from '$lib/components/NavigationSidebar.svelte';

	let { children } = $props();

	let pageTitle = $derived(getPageTitle($page.url.pathname, 'user'));
</script>
...
<Sidebar.Provider>
	...
	<Sidebar.Inset>
			...
			<div class="flex items-center gap-4">
				...
				<div class="flex items-center gap-2 rounded-full bg-gray-100 px-3 py-1">
					...
					<div class="flex flex-col text-left">
						<span class="text-sm font-semibold text-gray-800">Ludmila Sidorshina</span>
		...
		<!-- Page Content -->
		<div class="flex flex-1 flex-col gap-4 bg-[#f4f8fb] p-4 pt-0">
			{@render children()}
		</div>
	</Sidebar.Inset>
</Sidebar.Provider>
