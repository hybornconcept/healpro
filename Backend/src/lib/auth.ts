...
export const createAuth = async (db: PostgresJsDatabase<typeof schema>, env: any) => {
  return betterAuth({
    ...
    trustedOrigins: ["http://localhost:5173", "http://localhost:5174", "http://localhost:5175", "http://localhost:4173"], // Allow frontend origins
    plugins: [
      organization({
        schema: {
          organization: {
