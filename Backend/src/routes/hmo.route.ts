...
const hmoRoute = factory
  ...
  .post("/", zValidator("json", createHmoSchema), async (c) => {
    try {
      ...
      if (body.statesCovered)
        payload.statesCovered = JSON.stringify(body.statesCovered);
      if (body.planTypes) payload.planTypes = JSON.stringify(body.planTypes);
      if (body.hospitalPartners)
        payload.hospitalPartners = JSON.stringify(body.hospitalPartners);
      if (body.pharmacyPartners)
        payload.pharmacyPartners = JSON.stringify(body.pharmacyPartners);
      if (body.laboratoryPartners)
        payload.laboratoryPartners = JSON.stringify(body.laboratoryPartners);
      if (body.specialistPartners)
        payload.specialistPartners = JSON.stringify(body.specialistPartners);
      if (body.metadata) payload.metadata = JSON.stringify(body.metadata);

      const newHmo = await db.insert(hmos).values(payload).returning();
